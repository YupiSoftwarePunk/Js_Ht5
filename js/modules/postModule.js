...
export function initPostDetails(postsData) {
    try {
        ...
        console.error('Ошибка при инициализации деталей поста:', error);
    }
}


export function CreatePosts(data, storage, container = null)
{
    try {
        const targetContainer = container || document.querySelector('#post-list');
        ...
        likeBtn.addEventListener('click', (e) => {
            ...
            if (hasLike) {
                if (!currentLiked.includes(data.id)) currentLiked.push(data.id);
            } else {
